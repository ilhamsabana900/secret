...
app = Flask(__name__)
app.config['TEMPLATER_AUTO_RELOAD'] = True
app.config['UPLOAD_FOLDER'] = './static'

SECRET_KEY = 'SPARTA'

MONGODB_CONNECTION_STRING = 'mongodb+srv://ilham123:ilham123@cluster0.z5oy8if.mongodb.net/?retryWrites=true&w=majority'
...
#     # kita mendeskripsikan token yang sudah kita gunakan
#     try:
#         payload = jwt.decode(
#             token_receive,
#             SECRET_KEY,
...
#     # jika token tidak dapat diterjemahkan
#     except jwt.exceptions.DecodeError:
#         msg = 'tidak bisa masuk'
#         return redirect(url_for('login'))
...
def user(username):
    token_receive = request.cookies.get(TOKEN_KEY)
    # mendeskripsikan token yang sudah kita kunakan
    try:
        payload = jwt.decode(
            token_receive,
            SECRET_KEY,
        ...
        # kita membuat apakah pengguna yang masuk == profil ini atau buka
...
def sign_up():
    ...
    doc = {
        ...
        "profile_pic_real": "logo_pict.png", # a default profile image
...
def update_like():
    ...
    try:
        ...
        return jsonify({"result": "success", 'msg': 'terupdate', "count": count})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for('home'))
